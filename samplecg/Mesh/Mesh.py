...
def mesh(xFirst, xLast, zFirst, zLast, step):
    vertices, indices, colors, triangles = [], [], [], []
    # Calculating vertex list
    xMesh, zMesh = np.meshgrid(np.arange(xFirst, xLast + (xLast - xFirst) / step, (xLast - xFirst) / step),
                               np.arange(zFirst, zLast + (zLast - zFirst) / step, (zLast - zFirst) / step))
    # yMesh = xMesh ** 4 - 2 * xMesh ** 2 + zMesh ** 2
    yMesh = 4 * xMesh + 6 * zMesh - 2 * xMesh ** 2 - 2 * (zMesh ** 2) - 2 * xMesh * zMesh - 2 * (zMesh ** 2)
...
class Mesh(object):
    def __init__(self, vert_shader, frag_shader):
        self.vertices, self.indices, self.colors, self.normals = mesh(-2, 2, -2, 2,
                                                                      200)  # xFirst, xLast, zFirst, zLast, step
